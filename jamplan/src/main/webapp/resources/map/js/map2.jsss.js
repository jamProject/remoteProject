		...
		$.ajax({
			...
			success:function(data){ //성공적응답 받아오면 이 함수 자동실행 (data)가 컨트롤러에서 반환된 데이터가 들어있다.  // 변수이름은 바꿀수 있다.
				$.each(data, function(index, item){ // 이 데이터는 리스트니 하나의 데이터(peopleVO객체)가 아이템에 저장 / 인덱스는 고정된 값
				    ...
				    $('#placeName').html(item.placeName);
					$('#address').html(item.address);
					console.log("item.placeName=" + item.placeName);
				    infowindow.setContent($('#infoContent').html());				    
				    infoArray.push(infowindow);
				    
				    //pickList(item.placeName);
				    
				    $(".ui button:nth-child(" + (index+1) + ")").text(item.placeName);
				    /*
				    $("#map").on("mouseover", markersArray, function(){
			    ...
			    */
					...
					//}
					markersArray[index].addListener('click', function(mouseEvent){	
			        	console.log("autocomplete.addListene ");
   ...
   // this.infowindow = new google.maps.InfoWindow();
    ...
    ClickEventHandler.prototype.getPlaceInformation = function(placeId) {
    	 ...
    	 me.placesService.getDetails({placeId: placeId}, function(place, status) {
	    	...
	    	marker.addListener('mouseover',function(e,marker){
	        	console.log("autocomplete.addListene ");
